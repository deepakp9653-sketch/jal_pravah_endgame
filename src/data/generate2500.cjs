...
const stateData = {
  "Assam": { lat: 26.2006, lng: 92.9376, riskDist: [50, 30, 15, 5], total: 300 },
  "Bihar": { lat: 25.0961, lng: 85.3131, riskDist: [45, 35, 15, 5], total: 350 },
  "Uttar Pradesh": { lat: 26.8467, lng: 80.9462, riskDist: [30, 40, 20, 10], total: 380 },
  ...
  "Uttarakhand": { lat: 30.0668, lng: 79.0193, riskDist: [20, 40, 30, 10], total: 60 },
  ...
  // High-Density Microhotspots (Density Reduced per User Feedback)
  "Mumbai": { lat: 19.0760, lng: 72.8777, riskDist: [60, 30, 10, 0], total: 60, radius: 2.5, overrideState: "Maharashtra" },
  "Pune": { lat: 18.5204, lng: 73.8567, riskDist: [40, 40, 15, 5], total: 50, radius: 2.0, overrideState: "Maharashtra" },
  "Ratnagiri": { lat: 16.9902, lng: 73.3120, riskDist: [50, 40, 10, 0], total: 60, radius: 2.5, overrideState: "Maharashtra" }
};
...
function generateRandomCoords(state, baseLat, baseLng, radiusDeg = 2.0) {
  let latOffset = (Math.random() + Math.random() + Math.random() - 1.5) * radiusDeg;
  let lngOffset = (Math.random() + Math.random() + Math.random() - 1.5) * radiusDeg;
  
  // Strict Coastal Constraints to prevent rendering in the ocean
  const westCoast = ["Mumbai", "Ratnagiri", "Maharashtra", "Kerala", "Gujarat", "Karnataka"];
  const eastCoast = ["Odisha", "West Bengal", "Andhra Pradesh", "Tamil Nadu"];
  
  if (westCoast.includes(state)) {
    // Force longitude East (Landward), avoid severe western offsets into Arabian Sea
    lngOffset = Math.abs(lngOffset); 
  } else if (eastCoast.includes(state)) {
    // Force longitude West (Landward), avoid severe eastern offsets into Bay of Bengal
    lngOffset = -Math.abs(lngOffset);
  }
...
for (const [state, data] of Object.entries(stateData)) {
  for (let i = 0; i < data.total; i++) {
    const r = data.radius || (state === "Uttar Pradesh" || state === "Madhya Pradesh" ? 3.0 : 1.5);
    const coords = generateRandomCoords(state, data.lat, data.lng, r);
    ...
    const renderState = data.overrideState || state;

    massiveHotspots.push({
      id: idCounter++,
      name: (data.overrideState ? state : renderState) + " " + names[Math.floor(Math.random() * names.length)] + " " + Math.floor(Math.random() * 999),
      lat: parseFloat(coords.lat.toFixed(4)),
      lng: parseFloat(coords.lng.toFixed(4)),
      risk: risk,
      type: type,
      district: data.overrideState ? state : renderState,
      description: "Historical flood vulnerability micro-zone near " + state + " classified as " + risk + " risk based on severe rainfall events."
    });
  }
}

const fileContent = "export const massiveHotspots = " + JSON.stringify(massiveHotspots, null, 2) + ";\n";
